...
class vicsek3d(planktos.swarm):
    def __init__(self, *args, **kwargs):
        ...
        def __calc_dist(self, origin, positions_array, domain):
            ''' A private method that calculates the distance between the postion 
                    orgin and all the postions in postions_array
            ...
            x_bndry = envir.bndry[0][0]
            y_bndry = envir.bndry[1][0]
            z_bndry = envir.bndry[2][0]
            ...
            dist = np.sqrt(diffs[:,0]**2 + diffs[:,1]**2 + diffs[:,3]**2)
    ...
    def get_positions(self, dt, params):
        ...
        for n in range(self.positions.shape[0]):
            dist = self.__calc_dist(self.positions[n,:], self.positions, envir.L)
