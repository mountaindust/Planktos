...
# 2D channel flow with vortex shedding. Compare with results from 
#   ex_ib2d_ibmesh.py
...
class vicsek(planktos.swarm):
    def __init__(self, *args, **kwargs):
        ...
        self.shared_props['r'] = 1 # constant in Vicsek et al. (r=1)
        ...
        self.velocities += self.get_prop['v']*np.array([
            np.cos(rnd_angles), np.sin(rnd_angles)])
    ...
    def get_positions(self, dt, params):
        ...
        for n in range(self.positions.shape[0]):
            ...
            avg_angles[n] = np.arctan2(
                np.mean(self.velocities[dist<self.shared_props['r'],1]),
                np.mean(self.velocities[dist<self.shared_props['r'],0]))
        ...
        new_vel = self.get_prop['v']*\
            np.array([np.cos(new_angles), np.sin(new_angles)]) +\
