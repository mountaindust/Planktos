...
class environment:
    ...
    def read_IB2d_fluid_data(self, path, dt, print_dump, d_start=0, 
        ...
        dyload : bool, default=False
            whether or not to dynamically load the fluid data. Use this for 
            large data sets. Only supported for time-varying data.
        '''
...
class fCubicSpline(interpolate.CubicSpline):
    ...
    def __call__(self, val):
        if self.valid_times is not None:
            if (self.valid_times[0] is not None and val < self.valid_times[0])\
                or (self.valid_times[1] is not None and val > self.valid_times[1]):
...
class FluidData:
    ...
    def __init__(self, path, data_type, d_start, d_finish, LNUM=8,
        ...
        self.valid_time_range = (None, self.loaded_times[round(self.LNUM/2)])
