...
- The version lives in `planktos/__init__.py` (`__version__`); `setup.cfg` reads
  ...
  string in `__init__.py` is the single source of truth. `__version__` is `1.0.2`,
  which is **in development and not yet tagged**, accumulating bug fixes under its
  own `changelog.txt` section. `v1.0.1` (documentation-only) and `v1.0.0` are
  released and tagged.
